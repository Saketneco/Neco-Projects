        ...
        for custom_url in custom_urls:
            for fetched_url in self.fetched_urls:
                if self.is_pdf_url(fetched_url):
                    ...
                    if result:
                        print(f"Matched URL: {os.path.basename(custom_url)}#####{os.path.basename(fetched_url)}")
                        self.download_pdf(fetched_url)
...
if __name__ == "__main__":
    ...
    # Keeping files as a list of lists
    files = [
        'sow ingot', 
        'ingot', 
        'wire rod', 
    ...
    # Print similarity scores for custom URLs
